...
namespace PTL
{
    public class Math
    {
        ...
        protected static PointD Normalize(PointD r1)
        {
        ...
        protected static double GetLength(PointD p1)
        {
        ...
        protected static double[,] MatrixAdd(double[,] ptr1, double[,] ptr2)
        {
            int r1, c1;
            r1 = ptr1.GetUpperBound(0);  // UBound
            c1 = ptr1.GetUpperBound(1);  // UBound
        ...
        protected static double[,] MatrixAdd(double[,] ptr1, double[,] ptr2, double[,] ptr3)
        {
            int r1, c1;
            r1 = ptr1.GetUpperBound(0);  // UBound
            c1 = ptr1.GetUpperBound(1);  // UBound
            ...
            for (int i = 0; r1 >= i; i++)
            {
                for (int j = 0; c1 >= j; j++)
                {
                    outptr[i, j] = ptr1[i, j] + ptr2[i, j] + ptr3[i, j];
                }
            }
            return outptr;
        }
        ...
        protected static PointD Cross(PointD p1, PointD p2)
        {
        ...
        protected static double Dot(PointD p1, PointD p2)
        {
            double value;
        ...
        protected static PointD MatrixDot3(double[,] tMatrix, PointD tp1)
        {
            ...
            PointD p2 = new PointD(tr2[0], tr2[1], tr2[2]);
            return p2;
        }
        protected static PointD MatrixDot4(double[,] tMatrix, PointD tp1)
        {
        ...
        protected static PointD[] MatrixDot4(double[,] tMatrix, PointD[] points)
        {
            PointD[] ansPoint = new PointD[points.Length];
            for (int i = 0; i < points.Length; i++)
            {
                ansPoint[i] = MatrixDot4(tMatrix, points[i]);
            }
            return ansPoint;
        }
        ...
        protected static double[,] RotateMatrixArbitraryAxis(PointD tRotateAxis, double theta)
        {
            ...
            Cs[0, 2] = tRotateAxis.Y;
            Cs[1, 0] = tRotateAxis.Z;
            Cs[1, 1] = 0.0;
            Cs[1, 2] = -tRotateAxis.X;
            Cs[2, 0] = -tRotateAxis.Y;
            Cs[2, 1] = tRotateAxis.X;
            Cs[2, 2] = 0.0;
        ...
        protected static PointD RotatePointZ(PointD pnt, double theta)
        {
            PointD pntnew = MatrixDot4(RotateMatrix(Axis.Z, theta), pnt);
            return pntnew;
        }
        protected static PointD[,] RotatePointZ(PointD[,] pnt, double theta)
        {
            return RotatePoint(RotateMatrix(Axis.Z, theta), pnt);
        }
        ...
        protected static PointD RotatePoint(double[,] M21, PointD pnt)
        {
            PointD pntnew = MatrixDot4(M21, pnt);
            return pntnew;
        }
        protected static PointD[] RotatePoint(double[,] M21, PointD[] pset)
        {
            int nump = pset.Length;
            PointD[] pseta = new PointD[nump];
        ...
        protected static PointD[] TransformPoint(double[,] M21, params PointD[] pset)
        {
            Func<double[,], PointD, PointD> transformPoint = (double[,] tMatrix, PointD tp1) =>
                #region
                {
                    double[] tr1 = new double[4] { tp1.X, tp1.Y, tp1.Z, 1.0 };
                    double[] tr2 = new double[4];
            ...
            for (short k = 0; k < nump; k++)
                pseta[k] = transformPoint(M21, pset[k]);
        ...
        protected static PointD[,] RotatePoint(double[,] M21, PointD[,] pset)
        {
            ...
            PointD[,] pseta = new PointD[nump0, nump1];
        ...
        {
            public delegate double FunctionToDiff(double x);

            public static double Mid2PDiff(FunctionToDiff f, double xi, double h)
            {
