...
namespace PTL.Mathematics
{
    ...
    {
        protected const int maxIterations = 50;
        ...
        if (System.Math.Abs(fc) < System.Math.Abs(fb))
            ...
            tol = 2.0 * t * System.Math.Abs(b) + t;
            errorEstimate = m = 0.5 * (c - b);
            if (System.Math.Abs(m) > tol && fb != 0.0) // exact comparison with 0 is OK here
            {
                // See if bisection is forced
                if (System.Math.Abs(e) < tol || System.Math.Abs(fa) <= System.Math.Abs(fb))
                {
                    d = e = m;
                }
                else
                {
                    ...
                    if (p > 0.0)
                        ...
                        p = -p;
                    s = e; e = d;
                    if (2.0 * p < 3.0 * m * q - System.Math.Abs(tol * q) && p < System.Math.Abs(0.5 * s * q))
                        d = p / q;
                    else
                        d = e = m;
                }
        ...
        )
        {
            ...
            )
            {
                ...
                double absgprimex = System.Math.Abs(gprimex);
                if (absgprimex > 1.0 || System.Math.Abs(gx) < double.MaxValue * absgprimex)
                {
                    // The division will not overflow
                    newX = oldX - gx / gprimex;
                    errorEstimate = System.Math.Abs(newX - oldX);
                }
                else
                {
                    newX = oldX;
